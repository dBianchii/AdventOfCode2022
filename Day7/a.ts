...
class Directory {
	constructor(name: string, dirSize: number, parent: Directory | null, children: Directory[], files: File[]) {
...
lines.forEach((line) =>{
	if (line.substring(0,1) == '$'){
		...
		switch (command) {
			case 'cd':
				if (line.substring(5, 6) == "/"){
					...
					// Go to directory
					var gotoDir = currentDir?.children.find(x => x.name == line.substring(5, line.length))
					if (gotoDir != null)
						currentDir = gotoDir
				} else {
		...
		if (line.substring(0, 3) == 'dir'){
			//Is a directory
			const dir = new Directory(line.substring(4, line.length), 0, currentDir, [], [])
			...
			//Is a file
			const file = new Directory(line.split(' ')[1], Number(line.split(' ')[0]), currentDir, [], [])
			file.isFile = true
...
function getSize(node: Directory, directoryCallback: Function) {
	...
	const dirSize: number = node.children.map((child) => getSize(child, directoryCallback)).reduce((a, b) => a + b, 0)

	directoryCallback(node.name, dirSize)

	return dirSize
}

let countSmallFolders = 0
getSize(rootDir, (name: string, size: number) => {
	console.log({name, size})
	if (size < 100000){
		countSmallFolders += size
	}
})
console.log(countSmallFolders)